...
def top_results(i, bottom_session, hashmap_description_relevance, relevance_calculator, all_urls_to_search):
    ...
    with ThreadPool() as bottom_pool: # not specifying Pool(processes=__) so using max number of cores on computer
        ...
        bottom_pool_starmap = list(tqdm(bottom_pool.starmap_async(description_relevance_calculator, [(hashmap_description_relevance, relevance_calculator, bottom_session, tags_to_compare_to, urls_to_search[j]) for j in range(len(urls_to_search))]).get(), total=len(urls_to_search), desc="BOTTOM_POOL_STARMAP"))
...
def master_results(all_urls_to_search, dom_queue):
    ...
    with ThreadPool() as top_pool:
        ...
        top_pool_starmap = list(tqdm(top_pool.starmap_async(top_results, [(i, bottom_session, hashmap_description_relevance, relevance_analyzer.result_relevance_calculator, all_urls_to_search) for i in range(len(all_urls_to_search))]).get(), total=len(all_urls_to_search), desc="TOP_POOL_STARMAP"))
