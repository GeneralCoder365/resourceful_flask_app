...
def master_relevance_analyzer(all_urls_to_search, relevant_words_dict): # ! just return normally no need to make this a separate process
    try:
        ...
        # relevance_processes = []
        relevance_threads_parameters_array = []
        ...
        for relevance_thread_parameters in group_of_urls_to_search_results:
            relevance_threads_parameters_array += relevance_thread_parameters
        ...
        print("RELEVANCE PARAMETERS ARRAY LENGTH: ", len(relevance_threads_parameters_array))
        ...
        for param in relevance_threads_parameters_array:
            if (relevance_threads_parameters_array.count(param) > 1):
                print("DUPLICATE PARAMETERS: ", relevance_threads_parameters_array.index(param), " X ", relevance_threads_parameters_array.count(param))
        ...
        with ThreadPool() as top_pool:
            top_pool_starmap = top_pool.starmap_async(relevance_calculator, [param for param in relevance_threads_parameters_array]).get()
            top_pool.terminate()
        ...
        print("Error: " + str(e))
        print("Error on line {}".format(sys.exc_info()[-1].tb_lineno))
        print("ERROR IN MASTER RELEVANCE ANALYZER")
        print("Didn't work :(")
...
# ! IN PERSON TEST CASE
# tags = '{"skills": ["computer science", "cs", "math"], "interests": ["machine learning", "probability"], "type_of_opportunity": ["courses"], "in_person_online": "in person", "location": "Rockville MD USA"}'
