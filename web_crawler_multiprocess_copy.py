...
def top_urls_to_search(search_queries):
...
def master_urls_to_search(search_queries, dom_queue):
    ...
    for search_query in search_queries:
        if (search_query["in_person_online"] == "online"):
            search_queries_array[0].append(search_query)
        elif (search_query["in_person_online"] == "in person"):
            search_queries_array[1].append(search_query)
        else:
            print("SEARCH QUERY INVALID [IN PERSON ONLINE TYPE NOT ACCEPTABLE]: ", search_query)
    print("SEARCH QUERIES ARRAY: ", search_queries_array)
    
    urls_to_search = []
    if ((search_queries_array[0] != []) and (search_queries_array[1] != [])):
        print("WEB CRAWLER WILL RUN TWICE")
        print("RUNNING WEB CRAWLER FOR ONLINE QUERIES")
        urls_to_search += top_urls_to_search(search_queries_array[0])
        print("RUNNING WEB CRAWLER FOR IN PERSON QUERIES")
        urls_to_search += top_urls_to_search(search_queries_array[1])
    elif (search_queries_array[0] == []):
        print("WEB CRAWLER WILL RUN ONCE")
        print("RUNNING WEB CRAWLER FOR IN PERSON QUERIES")
        urls_to_search += top_urls_to_search(search_queries_array[1])
    elif (search_queries_array[1] == []):
        print("WEB CRAWLER WILL RUN ONCE")
        print("RUNNING WEB CRAWLER FOR ONLINE QUERIES")
        urls_to_search += top_urls_to_search(search_queries_array[0])
    else:
        print("SEARCH QUERIES ARRAY LENGTH INVALID: ", search_queries_array)
